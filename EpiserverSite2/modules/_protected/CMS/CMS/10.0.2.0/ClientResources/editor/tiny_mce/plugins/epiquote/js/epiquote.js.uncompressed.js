    ...
    */
    insert: function() {
        ...
        if (e == null) {
            ...
            // if we are editing a existing blockquote.
            if (node != null && node.nodeName == 'BLOCKQUOTE') {
                ed.dom.setAttribs(node, attrs);
            }               
            else {
    ...
    */
    remove: function() {
        var inst = tinyMCEPopup.editor,
            quoteTypes = document.forms[0].quoteType,
            elm, type, i;
        
        // Find the type of quote to remove.
        for (i = 0; i < quoteTypes.length; i++) {
            if (quoteTypes[i].checked) {
                type = quoteTypes[i].value;
            }
        }
        
        if (type) {
            elm = inst.dom.getParent(inst.selection.getNode(), type);
            tinyMCEPopup.execCommand("mceRemoveNode", inst, elm);
        }
