(function() {
	...
	function getScheme(src) {
		// Get the scheme from the input url or fallback to http
		var scheme = (/^([a-z]+):\/\//).exec(src);
		return scheme && scheme[1] || "http";
	}
	...
	window.Media = {
		...
		moveStates : function(to_form, field) {
			...
			if (to_form) {
				...
				// Get the scheme from the input url
				var scheme = getScheme(src);
				...
				if (src.match(/youtube\.com\/embed\/\w+/)) {
					...
					if (src.match(/youtu\.be\/[a-z1-9.-_]+/)) {
						...
						data.params.frameborder = '0';
						data.type = 'iframe';
						src = scheme + '://www.youtube.com/embed/' + src.match(/youtu.be\/([a-z1-9.-_]+)/)[1];
						setVal('src', src);
						setVal('media_type', data.type);
					}
					...
					if (src.match(/youtube\.com(.+)v=([^&]+)/)) {
						...
						data.params.frameborder = '0';
						data.type = 'iframe';
						src = scheme + '://www.youtube.com/embed/' + src.match(/v=([^&]+)/)[1];
						setVal('src', src);
						setVal('media_type', data.type);
					}
				}
				...
				if (src.match(/video\.google\.com(.+)docid=([^&]+)/)) {
					...
					data.type = 'flash';
					src = scheme + '://video.google.com/googleplayer.swf?docId=' + src.match(/docid=([^&]+)/)[1] + '&hl=en';
					setVal('src', src);
					setVal('media_type', data.type);
				}
				...
				if (src.match(/vimeo\.com\/([0-9]+)/)) {
					...
					data.params.frameborder = '0';
					data.type = 'iframe';
					src = scheme + '://player.vimeo.com/video/' + src.match(/vimeo.com\/([0-9]+)/)[1];
					setVal('src', src);
					setVal('media_type', data.type);
				}
				...
				if (src.match(/stream\.cz\/((?!object).)*\/([0-9]+)/)) {
					...
					data.params.frameborder = '0';
					data.type = 'iframe';
					src = scheme + '://www.stream.cz/object/' + src.match(/stream.cz\/[^/]+\/([0-9]+)/)[1];
					setVal('src', src);
					setVal('media_type', data.type);
				}
				...
				if (src.match(/maps\.google\.([a-z]{2,3})\/maps\/(.+)msid=(.+)/)) {
					...
					data.params.frameborder = '0';
					data.type = 'iframe';
					src = scheme + '://maps.google.com/maps/ms?msid=' + src.match(/msid=(.+)/)[1] + "&output=embed";
					setVal('src', src);
					setVal('media_type', data.type);
				}
