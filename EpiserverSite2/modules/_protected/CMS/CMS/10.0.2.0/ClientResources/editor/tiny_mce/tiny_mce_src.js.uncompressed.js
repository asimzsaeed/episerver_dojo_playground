...
(function(win) {
	...
	var tinymce = {
		...
		removeUnload : function(f, s) {
			var u = this.unloads, r = null;

			tinymce.each(u, function(o, i) {
				if (o && o.func == f && o.scope == s) {
					u.splice(i, 1);
					r = f;
					return false;
				}
	...
	function getHTML5() {
		...
		if (!html5) {
			html5 = mapCache.html5 = unpack({
					A : 'id|accesskey|class|dir|draggable|item|hidden|itemprop|lang|role|spellcheck|style|subject|tabindex|title|onclick|ondblclick|onmousedown|onmouseup|onmouseover|onmousemove|onmouseout|onkeypress|onkeydown|onkeyup',
					...
					'figcaption[A][C]' +
					'img[A|alt|src|height|width|usemap|ismap|longdesc][]' +
...
*/
		...
		destroy : function(s) {
			...
			if (!s)
				tinymce.removeUnload(t.destroy, t);
		},
	...
	tinymce.create('tinymce.dom.Selection', {
		...
		getRng : function(w3c) {
			...
			if (tinymce.isIE && !tinymce.isIE11 && rng && rng.setStart && doc.selection.createRange().item) {
			    ...
			    // IE sometimes returns a selection from the wrong document. Catching these cases now helps us avoid problems later.
			    if (elm.document === doc) {
			        rng = doc.createRange();
		...
		destroy : function(manual) {
			...
			// Manual destroy then remove unload handler
			if (!manual)
				tinymce.removeUnload(self.destroy, self);
		},
	...
	tinymce.dom.ScriptLoader = function(settings) {
		...
		this.loadScripts = function(scripts, callback, scope) {
			...
			loadScripts = function() {
				...
				if (!loading) {
				    var queueLoadedCallbacksCopy = queueLoadedCallbacks.slice(0);
				    // Reset the queue load callbacks before execute them. This will make it possible to load other scripts in the callback without messing up the queue.
				    queueLoadedCallbacks.length = 0;

				    tinymce.each(queueLoadedCallbacksCopy, function (callback) {
						callback.func.call(callback.scope);
					});
				}
			};
		...
		renderMenu : function() {
			...
			each(is(s.colors, 'array') ? s.colors : s.colors.split(','), function(c) {
				...
				var settings = {
					...
					'title': t.editor.getLang('colors.epi_' + c.toLowerCase(), c),
					'data-mce-color' : '#' + c
				};
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		destroy : function(s) {
		    ...
		    // Derefer from global settings
			if (tinymce.settings.url_converter_scope === t) {
			    delete tinymce.settings.url_converter_scope;
			}
...
(function(tinymce) {
	...
	tinymce.Editor.prototype.bindNativeEvents = function() {
		...
		var root = settings.content_editable ? self.getBody() : self.getDoc(),
	            body = self.getBody(),
                    form = self.getElement().form || tinymce.DOM.getParent(self.id, 'form');
		...
		each(nativeToDispatcherMap, function(dispatcherName, nativeName) {
			...
			switch (nativeName) {
				...
				case 'paste':
					dom.bind(body, nativeName, eventHandler);
					break;

				case 'submit':
				case 'reset':
					dom.bind(form || tinymce.DOM.getParent(self.id, 'form'), nativeName, eventHandler);
					break;
		...
		dom.bind(settings.content_editable ? self.getBody() : (tinymce.isGecko ? self.getDoc() : self.getWin()), 'focus', doOperaFocus);

		if (settings.content_editable && tinymce.isOpera) {
			dom.bind(body, 'click', doOperaFocus);
			dom.bind(body, 'keydown', doOperaFocus);
		}
	    ...
	    // Remove native events binding on removal to avoid memory leak
		self.onRemove.add(function (ed, e) {
		    each(nativeToDispatcherMap, function (dispatcherName, nativeName) {

		        switch (nativeName) {
		            case 'contextmenu':
		                dom.unbind(root, nativeName, eventHandler);
		                break;

		            case 'paste':
		                dom.unbind(body, nativeName, eventHandler);
		                break;

		            case 'submit':
		            case 'reset':
		                dom.unbind(form, nativeName, eventHandler);
		                break;
		    ...
		    // Set the editor as active when focused
		    dom.unbind(settings.content_editable ? self.getBody() : (tinymce.isGecko ? self.getDoc() : self.getWin()), 'focus', doOperaFocus);

		    if (settings.content_editable && tinymce.isOpera) {
		        dom.unbind(body, 'click', doOperaFocus);
		        dom.unbind(body, 'keydown', doOperaFocus);
		    }
...
(function(tinymce) {
	...
	tinymce.UndoManager = function(editor) {
		...
		function addNonTypingUndoLevel() {
		    var isIE = tinyMCE.isIE || tinyMCE.isIE11,
                fn = function () {
                    if (!editor.destroyed) {
                        self.typing = false;
                        self.add();
...
(function (tinymce) {
    ...
    tinymce.parseInt = function (string, radix) {
