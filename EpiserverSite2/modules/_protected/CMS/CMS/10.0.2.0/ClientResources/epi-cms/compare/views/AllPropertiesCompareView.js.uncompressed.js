define("epi-cms/compare/views/AllPropertiesCompareView", [
    ...
    return declare([FormEditing, _ModelBindingMixin], {
        ...
        _setRightMetadataAttr: function (metadata) {
            ...
            // Only recreate the form if there is new metadata for both left and right version.
            // If either the left or right metadata is not set then a context change is in progress
            // and we shouldn't recreate the form as that will be handled by the editing base.
            if (metadata && this.model.leftMetadata && this.viewModel) {
                this._setupForm();
            }
        },
