...
function (declare, lang, array, json, connect, domAttr, SHA1) {
    ...
    return declare(null, {
        ...
        update: function (mapping, patch) {
            ...
            lang.mixin(mapping, patch);
            if (patch.node) {
                mapping.nodeHash = this._calculateNodeHash(patch.node);
            }
        },
        ...
        remap: function (editableNodes) {
            ...
            array.forEach(this._mappings, lang.hitch(this, function (item) {
                ...
                if (haveMapped) {
                    ...
                    //delete block
                    item.updateController.destroy();
                    delete item.updateController;

                    //delete overlay item
                    if (item.overlayItem) {
                        item.overlayItem.destroy();
                        delete item.overlayItem;
                    }

                    delete item.node;
                    delete item.nodeHash;
                }

                if (this._isEmpty(item)) {
                    emptyMappingItems.push(item);
                }
            ...
            var emptyItem;
            while ((emptyItem = emptyMappingItems.pop())) {
                this._mappings.splice(array.indexOf(this._mappings, emptyItem), 1);
            }

            return unmappedNodes;
        },

        _isEmpty: function (mapping) {
