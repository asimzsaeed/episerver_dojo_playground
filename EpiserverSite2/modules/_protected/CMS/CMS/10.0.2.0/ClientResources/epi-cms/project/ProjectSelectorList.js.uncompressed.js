    ...
    return declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, _KeyNavContainer, _WidgetCommandBinderMixin], {
        ...
        buildRendering: function () {
            ...
            this.list = new (declare([OnDemandList, DijitRegistry, Selection, Keyboard, Focusable, Exclusive]))({
                ...
                className: "epi-project-selector-list__item-list epi-grid-max-height--300",
        ...
        renderRow: function (item, options) {
            ...
            var input = put("label", [put("span.dijitInline.dijitIcon.epi-radio-icon--secondary"), put("span.project-item--name.dojoxEllipsis", item.name)]);
            var state = put("div.project-item--status", epiDatetime.toUserFriendlyString(new Date(item.created)) + ", " + epiUsername.toUserFriendlyString(item.createdBy));
            return put("div", [input, state]);
        },
