...
﻿define(["dojo", "epi", "epi/dependency"], function (dojo, epi, dependency) {
    ...
    return dojo.declare(null, {
        ...
        //  _dependency: [private] Object
        //      Dependency resolver. Default is epi.dependency
        ...
        registerDependency: function (/*String*/name, /*Object|Function*/value, /*Function?*/initFunction, /*Function?*/unregisterFunction) {
            ...
            this._dependency.register(name, value, initFunction, unregisterFunction);
        },
        ...
        unregisterDependencies: function () {
            ...
            //
            dojo.forEach(this._registeredDependencies, function (entry, index) {
                this._dependency.unregister(entry);
            }, this);
            this._registeredDependencies = [];
            this._initialized = false;
        },
        ...
        resolveDependency: function (identifier) {
            ...
            return this._dependency.resolve(identifier);
        },
        ...
        unregisterDependency: function (identifier) {
            ...
            this._dependency.unregister(identifier);
            this._registeredDependencies = dojo.filter(this._registeredDependencies, function (item, index) {
...
function (epi, dojoDate, dateStamp, locale, lang, kernel, i18n) {
    ...
    return epi.datetime = {
        ...
        deserialize: function (dateString) {
            // summary:
            //      Deserialize the ISO formatted date string to a date object.
            // dateString: String
            //      An ISO formatted date string.
            // tags:
            //      public

            kernel.deprecated("epi/datetime", "Use Date.parse(dateString) or new Date(dateString)", "10.0");

            var rgex = /((?:\d{4})-(?:0[1-9]|1[012])-(?:0[1-9]|[12][0-9]|3[01])T(?:[01][0-9]|2[0123]):(?:[0-5][0-9])(?::(?:[0-5][0-9])(?:.\d+)?)?(?:(?:[+-](?:\d{2}):(?:[0-5][0-9]))|Z))/gi;
            if (!dateString) {
                return null;
            } else if (dateString.match && dateString.match(rgex)) {
                var date = dateStamp.fromISOString(dateString.match(rgex)[0]);
                return date;
            } else {
                throw "Invalid JSON date";
            }
        },

        serialize: function (date) {
            // summary:
            //    Serialize a javascript date object to ISO string.
            //
            // date: Date
            ...
            kernel.deprecated("epi/datetime", "Use Date.prototype.toISOString()", "10.0");

            return dateStamp.toISOString(date);
        },
        ...
        toUserFriendlyString: function (date, localeName, hideToday, excludeTime) {
            ...
            if (typeof date === "string") {
                ...
                // An invalid date is an "Invalid Date" object, which is best checked with isNaN.
                // TODO: Throw exception instead once we release CMS UI 10.0.
                if (!isNaN(parsedDate)) {
                    date = parsedDate;
                }
            }
...
﻿define(["dojo/_base/lang", "dojo/topic", "epi"], function (lang, topic, epi) {
    ...
    var _epiJQuery = lang.getObject("epiJQuery");
    ...
    //Registers jQuery for message sharing
    if (_epiJQuery !== undefined) {
        messaging.registerLibrary("jquery", _epiJQuery.event, "trigger",
        ...
        console.warn("epi/shell/messaging module was loaded but could not register epiJQuery for message sharing");
    }
...
'*now':function(r){r(['dojo/i18n!*preload*epi/nls/epi*["ar","ca","cs","da","de","el","en-gb","en-us","es-es","fi-fi","fr-fr","he-il","hu","it-it","ja-jp","ko-kr","nl-nl","nb","pl","pt-br","pt-pt","ru","sk","sl","sv","th","tr","zh-tw","zh-cn","ROOT"]']);}
}});
﻿define("epi/epi", [
    "dojo/_base/lang",
    "dojo/_base/array",
    ...
    epi.areEqual = function (first, second) {
        // summary:
        //		are the first and second objects/values deeply
        //		equivalent?
        ...
        if (lang.isArray(first) && lang.isArray(second)) {
            ...
            for (var x = 0; x < first.length; x++) {
                if (!epi.areEqual(first[x], second[x])) {
                    return false;
                }
            }
        ...
        if (((typeof first == "object") && ((typeof second == "object")))) {
            ...
            for (prop in first) {
                if (!epi.areEqual(first[prop], second[prop])) {
                    return false;
                }
            }
