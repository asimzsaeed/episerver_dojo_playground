...
function (connect, declare, lang, cookie, io, hash, StateFul, epi) {
    ...
    return declare([StateFul], {
        ...
        constructor: function (settings) {
            ...
            //      Subcribe [hashchange], [context/changed] event
            //
            // settings: Object
        ...
        getHash: function () {
            ...
            if (!currentHash) {
                var hashInCookie = cookie("epihash");
                if (hashInCookie) {
                    currentHash = hash(hashInCookie);
                }
                cookie("epihash", null, { expires: -1, path: "/" });
            }
        ...
        setHash: function (fragment) {
            ...
            hash(this.encodeEnabled ? encodeURIComponent(decodeURIComponent(io.objectToQuery(fragment))) : decodeURIComponent(io.objectToQuery(fragment)));
        },
        ...
        _onContextChange: function (ctx, callerData) {
            ...
            if (!this._hashAreEqual(storedHash, newHash)) {
                if (!callerData || callerData.sender !== this) {
                    ...
                    hash(this.extractHash(ctx, callerData));
                }
