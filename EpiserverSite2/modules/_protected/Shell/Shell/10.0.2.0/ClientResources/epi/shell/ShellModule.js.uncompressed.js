    ...
    return declare([_Module], {
        ...
        _requestInitialContext: function (hashWrapper, viewContextHistory) {
            ...
            when(viewContextHistory.getLastContextForView(), function (context) {
                var initialUri = null;
                if (context && context.fullHomeUrl) {
                    var url = new Url(context.fullHomeUrl);
                    // If the context originates from a different host, then discard it and use the default context
                    // This happens, for instance, when a page on another host is loaded, then its context is saved
                    // before switching hosts. It also happens if a previous context has been moved to another host.
                    if (url.authority.toLowerCase() === window.location.host.toLowerCase()) {
                        // Don't want to load specific versions from history since language parameters and such will then be ignored.
