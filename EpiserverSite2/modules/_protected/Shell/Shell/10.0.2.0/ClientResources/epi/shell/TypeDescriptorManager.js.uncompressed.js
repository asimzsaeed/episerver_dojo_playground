    ...
    typeResources
        ...
        //    public
            ...
            initialize: function () {
                ...
                return when(this.store.query()).then(lang.hitch(this, function (descriptors) {
                    return this.registerTypeSettings(descriptors);
                }));
            },
            ...
            registerTypeSettings: function (uiDescriptors) {
                ...
                function getBaseTypes(baseTypes) {

                    if (baseTypes && !(baseTypes instanceof Array)) {
                        baseTypes = [baseTypes];
                    }

                    return baseTypes ? baseTypes.map(function (baseType) {
                        return getOrCreateDeclaration(types[baseType]); //  eslint-disable-line no-use-before-define
                    }) : [];
                }

                function getOrCreateDeclaration(type) {

                    var uiDescriptor = type.descriptor,
                        baseTypes = uiDescriptor.baseTypes || uiDescriptor.baseTypeIdentifier;

                    return type.ctor ||
                        all(getBaseTypes(baseTypes)).then(function (baseTypes) {
                            type.ctor = declare(baseTypes, uiDescriptor);
                ...
                uiDescriptors.forEach(lang.hitch(this, function (uiDescriptor) {
                    this._prepareDescriptor(uiDescriptor);

                    types[uiDescriptor.typeIdentifier] = {
                        instance: null,
                        ctor: null,
                ...
                Object.keys(types).forEach(function (key) {
                    var type = types[key];
                    ctors.push(getOrCreateDeclaration(type));

                });

                return all(ctors).then(function () {
                    return true;
                });
            },
