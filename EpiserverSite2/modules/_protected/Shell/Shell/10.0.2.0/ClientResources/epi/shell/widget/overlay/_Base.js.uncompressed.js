...
function (
    ...
    return declare([_Widget], {
        ...
        _getPositionAttr: function () {
            ...
            if (oldPosition) {
                if (has("ie")) {
                    // IE can't measure(or round?) when scroll position is changing, so we cut it a slack of 1px
                    isChanged = Math.abs((oldPosition.x - position.x)) > 1 ||
                    Math.abs((oldPosition.y - position.y)) > 1 ||
                    Math.abs((oldPosition.w - position.w)) > 1 ||
                    Math.abs((oldPosition.h - position.h)) > 1;
                } else {
